#Natasha Mills          3-25-2025
#Assignment 4
# This assignment is to create a simple AI agent to use in your own coded version of the boardgame, Connect Four. By rewriting and adding in code in the desginated spots
# the game will then work as intended. This code runs a connect four game, with each function representing its different function for the game to work. 
        ...
        if row != -1:
            ...
            if row - 1 >= 0 and column - 1 >= 0:
                ...
                if board[row-1][column-1] == piece:
                    ...
                    print(f"Found adjacent at column {column}, row {row-1}, column {column-1}")
...
def checkWinner(board, playerNumber):
    ...
    for column in range(len(board[0]) - 3, 0):
...
def main():
    ...
    while running:
        ...
        if choice == "p":
            ...
            print("First, the user selects between one or two players.\n"
            "Each player then takes turns putting their pieces onto the board.\n"
            "Your goal is to connect four pieces in an unbroken sequence.\n"
            "A sequence can be horizontal, vertical, or diagonal.\n"
            "The game ends when one player has... FOUR IN SEQUENCE!\n")
            pass
        elif choice == "q":
            running = False
            print("Goodbye! Thank you for playing Four In Sequence!")
        else:
